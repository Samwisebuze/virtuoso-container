...
export class K8Api {
    ...
    static async getDeploymentContainers(deploymentId: string): Promise<object[]> {
        ...
        return allContainers.filter(entry => {
            ...
            if (selectors.deploymentId && selectors.deploymentId === deploymentId) {
                return true
            }
     ...
     * The name for each container is the machineId given by the network
     * definition.
     * 
     ...
     */
    static async createDeployment(containerConfigs: DeploymentContainer[], networkId: string): Promise<[string, string?]> {
        ...
        // TODO: add container for networking service: responsible for routing
        //       requests around the software-defined network
    ...
    static async execContainer(containerId: string, userCommand: string): Promise<string> {
